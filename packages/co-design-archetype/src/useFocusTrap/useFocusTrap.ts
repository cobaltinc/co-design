      ...
      if (node) {
        const processNode = (_node: HTMLElement) => {
          ...
          if (focusElement) {
            ...
            console.warn('[@co-design/archetype/useFocusTrap] Failed to find focusable element within provided node', node);
          }
        };
        ...
        setTimeout(() => {
          if (node.ownerDocument) {
            ...
            // eslint-disable-next-line no-console
            console.warn('[@co-design/archetype/useFocusTrap] Ref node is not part of the dom', node);
          }
        });
